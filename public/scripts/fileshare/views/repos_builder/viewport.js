    ...
    spawn_context_action([{
        ...
        action: () => {
            if (item.is_directory) {
                window.open(`/fileshare/repos/${current_repos.access_key}/archive/?directory=${item.absolute_path()}`, '_blank').focus();
            }
            else
        ...
        action: async () => {
            let url;
            if (item.folders) {
                url = `${location.hostname}/fileshare/repos/${current_repos.access_key}/?directory=${item.absolute_path()}`;
            }
            else {
                url = `${location.hostname}/fileshare/repos/${current_repos.access_key}/file/${item.id}`;
            }
