...
class StatsViewport extends AppWidget {
    ...
    connectedCallback() {
        this.set_content(require('./stats_viewport.hbs'), {}, {
    ...
    async refresh_data() {
        ...
        for (const [cat_name, category] of Object.entries(stats.values)) {
            ...
            cat.append(fields)
            this.elements().categories.append(cat);
        }
        ...
        this.elements().workload_bar.style.width = `${stats.cpu_usage * 100 / this.max_workload}%`;
        this.elements().workload_txt.innerText = Math.trunc(stats.cpu_usage * 10000) / 100 + `/${Math.trunc(this.max_workload * 100)}%`;

        this.elements().ram_bar.style.width = `${stats.ram_used / stats.ram_total * 100}%`;
        this.elements().ram_txt.innerText = `${humanFileSize(stats.ram_used)} / ${humanFileSize(stats.ram_total)}`;

        this.elements().swap_bar.style.width = stats.swap_total === 0 ? '0' : `${stats.swap_used / stats.swap_total * 100}%`;
        this.elements().swap_txt.innerText = `${humanFileSize(stats.swap_used)} / ${humanFileSize(stats.swap_total)}`;

        this.elements().cpus.innerHTML = '';
        let num = 0;
        for (const [usage, freq] of stats.cpus) {
            ...
            this.elements().cpus.append(cpu_div)
        }

        this.elements().disks.innerHTML = '';
        for (const [name, disk] of Object.entries(stats.disks)) {
            ...
            this.elements().disks.append(disk_div)
        }

        this.elements().networks.innerHTML = '';
        for (const [name, [up, down]] of Object.entries(stats.network)) {
            ...
            this.elements().networks.append(network_div)
        }
    }
}
