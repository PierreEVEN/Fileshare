...
import {User} from "../../../src/remote_filesystem/user";
import {edit_user} from "../../modals/edit_user/edit_user";
import {APP_COOKIES} from "../../modals/cookies/cookies";
import {human_readable_timestamp} from "../../../src/utilities/utils";
...
class UserViewport extends AppWidget {
    ...
    async _fill_data() {
        ...
        this.set_content(require('./user_viewport.hbs'), {
        ...
        for (const repository of repository_ids) {
            this.elements().repository_list.append(document.createElement('repository-tree-button').set_repository(this.get_app().pool.find_repository(repository)));
        }
        ...
        if (this.user === this.get_app().state.connected_user()) {
            ...
            for (const token of tokens) {
                ...
                this.elements().tokens.append(div);
            }
        }
    }
}
