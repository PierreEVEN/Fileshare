...
async function finalize_file_upload(file_path, metadata, repos, user, file_hash) {
    ...
    const parent_item = (await Directories.find_or_create(repos.id, meta.virtual_path, {owner: user.id}));
...
router.post('/', async (req, res) => {
    ...
    req.on('end', async () => {
        if (upload_in_progress[file_id].received_size >= upload_in_progress[file_id].metadata.file_size) {
            logger.info(`${req.log_name} store '${JSON.stringify(upload_in_progress[file_id].metadata)}' to repos '${req.display_repos.display_name}'`)
